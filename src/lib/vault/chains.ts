...
/** Link positions along the four corner-to-hub chains (skipping the padlock). [x, y, odd] */
...
export function drawChains(V: Vault, x: Ctx2D){
  ...
  for(let si=0;si<4;si++){ if(!S.chains[si]) continue; const [[x0,y0],[x1,y1]]=G.chains[si], nx=-(y1-y0), ny=x1-x0, nl=Math.hypot(nx,ny);
...
export function snapChain(V: Vault, i: number){ const { S, geo: G, env } = V; if (!S.chains[i]) return; S.chains[i]=false;
  const ox=S.cx-G.hw, oy=S.cy-G.hh, [[x0,y0],[x1,y1]]=G.chains[i], outx=(x0+x1)/2-G.hw, outy=(y0+y1)/2-G.hh, ol=Math.hypot(outx,outy)||1;
