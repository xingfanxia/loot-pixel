...
function drawCoin(V: Vault, x: number,y: number,ph: number){ const g=V.B.g, f=Math.floor(ph)%4, sw=[1,.6,.18,.6][f]; x=Math.round(x); y=Math.round(y);
...
/** Everything behind the card: lit dust, altar shadow, coins, void stars, bricks, links, summoning ring, torches, sucks. */
export function drawWorld(V: Vault, ox: number, oy: number, c: number){
  const { S, L, FX, U, geo: G, spr } = V, g=V.B.g, { CX, PTOP, TORCH } = L;
  // dust motes (lit)
  ...
  // torch halos + flames
  for (const t of TORCH){ const fl=1+.15*Math.sin(S.t*13)+S.torchBoost*.6, x=t.x+ox, y=t.y-3+oy; for(const [r,k,l] of [[Math.round(7*fl),'Y',.35],[Math.round(4*fl),'y',.6],[2,'o',.9]] as [number,string,number][]){ g.fillStyle=pat(V,g,k,l); for(let yy=-r;yy<=r;yy++){ const hw=Math.round(Math.sqrt(r*r-yy*yy)); g.fillRect(x-hw,y+yy,hw*2+1,1); } } }
