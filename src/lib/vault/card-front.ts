...
/** The vault theme's backdrop behind the art: dithered radial glow in the tier colours, floor strip, star seeds. */
export function vaultArtBg(V: Vault, vr: number){
...
/** Card face for the visible tier: the theme's frame, the art and its sparkles, the theme's chrome, then face cracks and the upgrade glitch. */
export function drawFront(V: Vault, t: number){
  const { S, geo: G, B, theme } = V, x=B.frontG, CWd=G.w, CHd=G.h, a=G.art, R=TIERS[S.vr]; x.clearRect(0,0,CWd,CHd);
  theme.face.frame(V, t);
  drawArt(V, t);
  ...
  theme.face.overArt?.(V, t);
  theme.face.chrome(V, t);
...
/** The vault theme's frame: bevelled tier-colour rim (colour-cycling on top tiers), corner studs, dark inner panel. */
export function vaultFrame(V: Vault, t: number){
  const { geo: G } = V, x=V.B.frontG, CWd=G.w, CHd=G.h, R=TIERS[V.S.vr];
...
/** The vault theme's chrome: art underline, title bar, nameplate, stat bars, one gem per ladder step. */
export function vaultChrome(V: Vault, _t: number){
  const { S, geo: G } = V, x=V.B.frontG, a=G.art, vr=S.vr, R=TIERS[vr];
...
export function drawTitleBar(V: Vault, col: string){
...
/** Art window: backdrop + twinkling stars, a halo, then the sprite (bobbing) or the portrait (restyled by the theme if it does), popping in on reveal. */
function drawArt(V: Vault, t: number){
  const { S, geo: G, B, theme } = V, x=B.frontG, a=G.art, R=TIERS[S.vr], card=S.face ?? S.spec!, assets=V.art.get(card.id)!, pic=theme.face.portrait ? theme.face.portrait(V, assets.art) : assets.art;
  ...
  if (sprite){
    ...
    const sz=Math.round(ss*ps); x.drawImage(pic,Math.round(hx-sz/2),Math.round(G.spriteCy-sz/2)+bob,sz,sz);
  } else {
    const img=pic, w=Math.round(img.width*ps), h=Math.round(img.height*ps);
