...
import { buildBackBase } from './card-back';
...
/**
 * Boots the vault scene for `deck` on the given DOM nodes. The deck's art is preloaded first
 * (missing images become placeholders), then the loop starts. destroy() tears everything down,
 * so a React unmount (or StrictMode's double mount) is clean.
 */
export function createVault(els: VaultElements, deck: Deck, hooks: VaultHooks = {}): VaultController {
  ...
  const reduce=matchMedia('(prefers-reduced-motion: reduce)').matches, APP: DebugHandles = window.APP ?? {}, A=new ChipAudio();
  ...
  const V: Vault = { env, L: initialLayout(), S: createState(geo.bars.length), FX: createFx(), U: emptyUnits(), B: buffers(els, geo.w, geo.h, geo.art.w, geo.art.h, geo.t1w, geo.t1h),
    ...
    A, spr: buildSprites(), deck, geo, ladder: deck.tiers.map(t=>t.tier), art: new Map(), bag: openBag(deck) };
  ...
  loadDeckArt(deck, V.spr, geo).then(art=>{
    ...
    buildBackBase(V); applyLayout(V); startEnter(V);
    ...
    Object.assign(APP,{ S: V.S, FX: V.FX, L: V.L, geo, force:(r: number)=>{V.S.force=r;}, step:(d: number)=>tick(V,d), render:()=>render(V),
      ...
      deck: { id: deck.id, tiers: V.ladder.map(t=>({ id: t, name: TIERS[t].name })), cards: deck.cards.map(c=>({ id: c.id, slot: c.slot, tier: c.tier, tierName: TIERS[c.tier].name, name: c.name })) } });
