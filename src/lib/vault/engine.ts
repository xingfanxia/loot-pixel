...
function buffers(els: VaultElements, cw: number, ch: number, aw: number, ah: number, t1w: number, t1h: number): Buffers {
  ...
  const [brC,brG]=mk(32,32), [dissC,dissG]=mk(cw,ch), [backC,backG]=mk(cw,ch,true), [frontC,frontG]=mk(cw,ch), [tmpC,tmpG]=mk(cw,ch), [artC,artG]=mk(aw,ah), [t1C,t1G]=mk(t1w,t1h), [tileSrcC,tileSrcG]=mk(cw,ch), [rotC,rotG]=mk(16,16);
